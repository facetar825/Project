...
func run() -> void:
	...
	game.start_run()
	check(game.stage == 0 and game.remaining == 0, "No enemies before room entry")
	...
	for expected_stage in range(1, 4):
		...
		game._process(0.02)
		check(game.waiting_for_entry and game.stage == expected_stage, "Next room waits for player entry")
		game.hero.position = game.dungeon.checkpoint(expected_stage)
		game._process(0.01)
	...
	while not game.dungeon.ready_for_play:
		await physics_frame
	check(game.mode == "playing" and game.kills == 0 and game.hero.level == 1 and game.stage == 0, "Restart resets run")
	game.hero.invulnerability = 0
